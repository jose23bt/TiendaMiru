...
// ═══════════════════════════════════════
// OBTENER NÚMERO SECUENCIAL DE PEDIDO
// ═══════════════════════════════════════
async function obtenerNumeroPedido() {
  const ref = db.collection("config").doc("pedidoCounter");
  const doc = await ref.get();
  ...
  // Guardar el nuevo número incrementado
  await ref.set({ 
    numero, 
...
exports.crearPreferencia = onCall(
  ...
  async (request) => {
    ...
    const modalidad = request.data.modalidad || "retiro"; // ✅ NUEVO
    ...
    try {
      // ✅ NUEVO: Obtener número secuencial
      ...
      const pedidoData = {
        numero: numeroPedido, // ✅ NUEVO: número secuencial
        ...
        estado: "pendiente",
        metodo: "mercadopago",
        entrega: modalidad === "delivery" ? "delivery" : "retiro", // ✅ MODIFICADO
        ...
        cliente: cliente ? {
          ...
          direccion: sanitize(cliente.direccion || "", 300), // ✅ Ya estaba
          email: sanitize(cliente.email || "", 100),
          uid: sanitize(cliente.uid || "", 50),
        } : {
          // ✅ NUEVO: cliente anónimo por defecto
...
//  Endpoints públicos para indexación. No modifican la lógica existente.
//  Mapear en firebase.json (rewrites) a /robots.txt y /sitemap.xml.
// ─────────────────────────────────────────────

exports.robots = onRequest({ region: REGION }, (req, res) => {
...
exports.sitemap = onRequest({ region: REGION }, (req, res) => {
  const lastmod = new Date().toISOString().split("T")[0];
  const urls = [
    { loc: `${BASE_URL}/`, priority: "1.0", changefreq: "weekly" },
    { loc: `${BASE_URL}/#comida-lista`, priority: "0.9", changefreq: "weekly" },
